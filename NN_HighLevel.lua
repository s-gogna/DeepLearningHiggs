...
-- Build the neural net architecture for HighLevel (High)
local model = nn.Sequential()
model:add(nn.Linear( 7, 150 ))
model:add(nn.PReLU())
...
for i = 1, 8 do
	...
	local trainData = readFile( filenames[ fileShuffle[i] ], false, true )
	print( 'Training ( ' .. i .. ' / 8 ) . . .' )
...
-- Test the model
local testData = readFile( "Higgs_Test.csv", false, true )
print( "Testing . . ." )
