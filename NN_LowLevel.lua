...
-- Build the neural net architecture for LowLevel (Low)
local model = nn.Sequential()
model:add(nn.Linear( 21, 150 ))
model:add(nn.PReLU())
...
for i = 1, 8 do
	...
	local trainData = readFile( filenames[ fileShuffle[i] ], true, false )
	print( 'Training ( ' .. i .. ' / 8 ) . . .' )
...
-- Test the model
local testData = readFile( "Higgs_Test.csv", true, false )
print( "Testing . . ." )
